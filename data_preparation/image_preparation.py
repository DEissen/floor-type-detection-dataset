...
def remove_obsolete_images(measurement_path, camera_name, earliest_timestamp):
    """
    ...
    restart_for_right_camera_image_flag = False
    ...
    for index, cam_file in enumerate(cam_files):
        ...
        if not restart_for_right_camera_image_flag and current_filename[0] == "R":
            reference_timestamp = earliest_timestamp
            restart_for_right_camera_image_flag = True

        if current_timestamp < earliest_timestamp:
            # remove all files before the earliest timestamp
            os.remove(cam_file)
        elif (current_timestamp > earliest_timestamp) and (camera_name == "BellyCam"):
